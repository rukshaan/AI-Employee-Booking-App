...
const Employment = () => {
  ...
  const createBooking = async () => {
    if (!selectedEmployer || !selectedWorkType || !bookingTitle) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>Select Employer:</Text>
      <FlatList
        horizontal
        data={employers}
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={[styles.employerCard, selectedEmployer?.id === item.id && styles.selectedCard]}
...
const styles = StyleSheet.create({
  ...
  employerCard: { padding: 12, borderWidth: 1, borderColor: "#ddd", borderRadius: 8, marginRight: 8, minWidth: 150 },
  ...
  employerName: { fontWeight: "bold", fontSize: 16 },
